...
$jscomp.getGlobal=function(a){return"undefined"!=typeof window&&window===a?a:"undefined"!=typeof global&&null!=global?global:a};$jscomp.global=$jscomp.getGlobal(this);
...
a=this.offset();"html"!==this.get(0).nodeName&&(d=f.offset());d.top+=b(f.css("borderTopWidth"));d.left+=b(f.css("borderLeftWidth"))}return{top:a.top-d.top-this.css("marginTop"),left:a.left-d.left-this.css("marginLeft")}};e.text=function(a){if("undefined"!==typeof a)return this.elements.forEach(function(c){c.innerText=a}),this;text="";this.elements.forEach(function(a){text+=a.innerText});return text};e.size=function(){return this.elements.length};e.data=function(a,b){var c=this.get(0).dataset;c=c||
{};if(a&&b)c[a]=b;else return a?c[a]:c};e.scrollTop=function(a){var c=this.get(0);if(!c)return null;void 0!==a&&(this.get(0).scrollTop=a);return c.scrollTop};$.extend=function(a){Array.prototype.slice.call(arguments,1).forEach(function(c){for(var b in c)a[b]=c[b]});return a};var p=function(a){var c=a.method.toUpperCase().trim(),b=a.data,d=a.url;if(b instanceof JQLite){var e=b.elements;b={};e.forEach(function(a){a.name&&(b[a.name]=a.value)})}if("string"!==typeof b&&!(f.FormData&&b instanceof FormData)){e=
[];for(var m in b)"GET"===c?d=updateURLVar(d,m,b[m]):e.push(m+"="+encodeURIComponent(b[m]));b=e.join("&")}var g=new XMLHttpRequest;g.onreadystatechange=function(){if(g.readyState===XMLHttpRequest.DONE){if(200===g.status){var c=g.response||g.responseText;"json"===a.dataType&&(c=JSON.parse(c));a.success(c)}else a.error(g,g.status,g.response);a.complete()}};g.open(c,d,!0);"GET"!==c&&a.contentType&&g.setRequestHeader("Content-type",a.contentType);a.beforeSend(a);g.send(b)};$.ajax=function(a){var c={success:function(){},
error:function(){},url:f.location.href,method:"GET",complete:function(){},beforeSend:function(){},dataType:"html",data:"",contentType:"application/x-www-form-urlencoded"};$.extend(c,a);p(c)};e.load=function(a,b,d){var c,f=this,e=a.indexOf(" ");if(-1<e){var g=a.slice(e).trim();a=a.slice(0,e)}b&&"object"===typeof b&&(c="POST");$.ajax({url:a,method:c||"GET",dataType:"html",data:b,success:function(a){f.html(g?$("<div>").append(a).find(g):a);d&&d(a)}})};$.get=function(a,b,d){$.ajax($.extend({url:a,success:b},
...
function getURLVar(a){var b={},d=String(document.location).split("?");if(d[1]){d=d[1].split("&");for(i=0;i<d.length;i++){var f=d[i].split("=");f[0]&&f[1]&&(b[f[0]]=f[1])}if(b[a])return b[a]}return""}function updateURLVar(a,b,d){var f=new RegExp("([?&])"+b+"=.*?(&|$)","i"),g=-1!==a.indexOf("?")?"&":"?";a.match(f)&&d?a=a.replace(f,"$1"+b+"="+d+"$2"):a.match(f)&&!d?a=a.replace(f,"$1"):d&&(a=a+g+b+"="+d);if(a.endsWith("&")||a.endsWith("?"))a=a.substr(0,a.length-1);return a}
...
b,"sharer","toolbar=0,status=0,width=660,height=445");break;case "pinterest":window.open("http://www.pinterest.com/pin/create/button/?media="+f+"&url="+b,"sharerpinterest","toolbar=0,status=0,width=660,height=445")}}function isMobile(){return 768>window.innerWidth}
function showMsg(a,b,d){b||(b="success");d=d||{};mgs_type=d.mgs_type?d.mgs_type:app.mgs_type;"popup"===mgs_type?(b=new Popup(a,"mgs-popup "+b),d=d.duration||5,b.render(),setTimeout(function(){},1E3*d)):($(".alert, .text-danger").remove(),d=$(".container.alert-container"),0===d.size()&&(d=$('<div class="container alert-container"></div>'),$(".after-header").after(d)),a=$('<div class="alert alert-'+b+'">'+a+'<button type="button" class="close" data-dismiss="alert">x</button></div>'),d.append(a),a.find(".close").on("click",
function(){a.remove()}),$("body, html").scrollTo(0,600))}
$(function(){$(".text-danger").each(function(){$(this).parents(".form-group").addClass("has-error")});var a=$("#search input[name='search']");a.parent().find("button").on("click",function(){
	var b=$("base").attr("href")+"product/search",d=a.val(),h=[];
	d&&h.push("search="+encodeURIComponent(d));
	(d=$("#search select[name='category_id']").val())&&h.push("category_id="+d);
	h&&(b=b+"?"+h.join("&"));
	location=b;
	fbq("track","Search")});
a.on("keydown",function(b){13===b.keyCode&&a.parent().find("button").trigger("click")});
$(document).delegate(".checkout-btn","click",function(){
	try{fbq("track","InitiateCheckout")}catch(f){}
});
var b=$(".mc-toggler");b.on("click",function(){b.is(".loaded")/*||(cart.reload(),b.addClass("loaded"))*/});var d=$(".cmpr-toggler");d.on("click",function(){d.is(".loaded")||(compare.reload(),d.addClass("loaded"))})});
...
var cart={
		...
		$.ajax({
			...
			success:function(b){
					...
					b.success&&(showMsg(b.success),
						...
						fbq("track","AddToCart",{
							content_ids:[a],
							content_type:"product",value:b.item_total,currency:"BDT"
						})
						)}
				})
	},
	update:function(a,b){
		...
		var d=this;
			...
			$.ajax({
				...
				success:function(a){setTimeout(function(){
	...
	},reload:function(){var a=$("#m-cart");a.append($("<div class='mask'></div>"));


$.get("common/cart/info",
	function(b){
		b=$(b);
		a.find(".content").size()?(a.find(".content").html(b.find(".content").html()),
		a.find(".footer").html(b.find(".footer").html())):a.html(b);
		a.find(".mask").remove();app.event.trigger("cart_load",[a])
	})
}
	...
	compare={
			...
			$.fn.autocomplete=function(a){
				return this.each(function(){
					...
					b.on("focus",function(){d.request()});
					b.on("blur",function(){setTimeout(function(a){a.hide()},200,this)});

					b.on("keydown",function(a){
						switch(a.keyCode){
							case 27:d.hide();
							break;default:d.request()}
					});
					d.click=function(a){(value=$(a.target).parent().attr("data-value"))&&this.items[value]&&(a.preventDefault(),
				this.select(this.items[value]))};
					d.show=function(){
						...
						d.show()};
						d.hide=function(){b.siblings(".dropdown-menu").hide()};
						this.request=function(){clearTimeout(d.timer);
							d.timer=setTimeout(function(a){a.source($(a).val(),
								a.response.bind(a))},500,this)};
							d.response=function(a){
								var f="";
								if(a.length){
									for(i=0;i<a.length;i++)d.items[a[i].value]=a[i];
										for(i=0;i<a.length;i++)a[i].category||(f+='<li data-value="'+a[i].value+'"><a href="#">'+a[i].label+"</a></li>")}f?d.show():d.hide();
...
$.fn._scrollTo=function(a,b){
	...
	f=null;if(1<d.size()){
		for(var g=0;g<d.size();g++){
			var h=d.elements[g];
			if(h.scrollHeight>h.clientHeight){f=h;break}}f&&(d=$(f))}else f=d.get(0);
			if(null==f)this.each(function(){this.scrollTop=a});
			else if(!(0>=b||0===d.size())){
				var l=(a-f.scrollTop)/b*10;
				setTimeout(function(){
					f.scrollTop+=l;
					f.scrollTop!==a&&this._scrollTo(a,b-10)}.bind(d),10)
			}
		};

		$.fn.scrollTo=function(a,b){try{
			window.scroll({top:a,behavior:"smooth"})
		}catch(d){window.scroll(0,a)}};

	function Popup(a,b){
		var d=this;
		this.el=a=$('<div class="popup '+b+'"><div class="popup-inner">'+a+'<span class="popup-close"></span></div></div>');
		a.find(".popup-close,.close").on("click",function(){d.close()});
		a.on("click",function(a){$(a.target).is(".popup")&&d.close()})
	}

	var _p=Popup.prototype;_p.render=function(){
		this.timer&&clearTimeout(this.timer);
		this.el.addClass("f-in").removeClass("f-out");$("body").append(this.el)
	};

_p.close=function(){
	var a=this;a.el.addClass("f-out").removeClass("f-in");
	a.timer=setTimeout(function(){a.el.remove()},2E3)
};
...
function Tab(a){
	...
	b.headers.each(function(){
		...
		f.on("click",function(){b.activate(f)});
		f.is(".active")&&(d=f)});null==d&&(d=$(b.headers.get(0)));
	b.activate(d)
}
...
_t.activate=function(a){
	this.active&&(this.active.removeClass("active"),
		this.elm.find("#"+this.active.attr("data-tab")).hide());
	this.elm.find("#"+a.attr("data-tab")).show();a.addClass("active");
	this.active=a
};(function(){function a(a,b){
	var d=new Image,e=a.getAttribute("data-src");
	d.onload=function(){a.parent?a.parent.replaceChild(d,a):a.src=e;
		b?b():null};
		d.src=e
	}function b(a){
		a=a.getBoundingClientRect();
		return 0<=a.top&&0<=a.left&&a.top<=(window.innerHeight||document.documentElement.clientHeight)}var d=[],f=function(){for(var f=[],h=0;h<d.length;h++)b(d[h])&&(a(d[h]),f.push(d[h]));
			f.forEach(function(a){a=d.indexOf(a);-1<a&&d.splice(a,1)})};
			$(function(){$("img.lazy").each(function(){d.push(this)});
				f();
window.addEventListener("scroll",f)})})();
$(function(){function a(a){
	var b="";
	a.forEach(function(a){
		b="remainder_cnt"===a.type?b+('<div class="search-item remainder-count"><a href="'+a.href+'">'+a.label+"</a></div>"):b+('<div class="search-item"><a href="'+a.href+'"><div class="image"><img src="'+a.thumb+'"></div><div class="name">'+a.label+"</div>"+(a.price?a.special?'<div class="price"><ins>'+a.special+"</ins> &nbsp; <del>"+a.price+"</del></div>":'<div class="price">'+a.price+"</div>":"")+"</a></div>")});return b}var b=$('#search input[name="search"]'),
d=$('<div class="dropdown-menu">\n\t\t<div class="search-details">\n\t\t    <ul class="nav nav-tabs">\n                <li data-tab="tab-prod">Products</li>\n                <li data-tab="tab-cat">Categories</li>\n                <li data-tab="tab-man">Brands</li>\n            </ul>\n            <div id="tab-prod" class="search-results"></div>\n            <div id="tab-cat" class="search-results"></div>\n            <div id="tab-man" class="search-results"></div>\n\t\t</div>\n\t</div>');
/*b.autocomplete({wrap:d,source:function(b,g){
	var c=this;
	b?$.ajax({url:"common/search_suggestion/index?keyword="+encodeURIComponent(b),
		dataType:"json",
		success:function(b){
			var e=a(b.manufacturers);
			d.find("#tab-man").html(e?e:'<div class="search-item empty">No results found for query</div>');
			e&&f.activate(d.find("[data-tab=tab-man]"));
			e=a(b.categories);
			...
			e&&f.activate(d.find("[data-tab=tab-prod]"));e?c.show():c.hide();
			d.scrollTop(0)}
		}):c.hide()},select:function(a){}});

	var f=new Tab(d),g=!1,h=b.get(0),l=h.hide;
	h.hide=function(){g||l()};
	$(document).on("click",function(a){a=$(a.target);
...
$(function(){function a(a){
	var b=this;a.forEach(function(a){var c=$(a.toggle);
		c.on("click",function(){c.hasClass("close")?b.hide(a):b.show(a)})}
		)}
	function b(a){
		var b=this;
		b.elm=a;
		...
		a.find(".slide").each(function(a,d){d=$(d);
		b.slides[a]=d;
		var e=$("<span>",{"class":"dot"});
		c.append(e);
		b.dots[a]=e;
		d.hide();
		e.on("click",function(){b.showSlides(a)})});
		a.append(c);
		b.index=0;
	b.showSlides(0)
}
...
var g=$("html"),h=$("body"),l=$(".overlay"),e=a.prototype;
e.show=function(a){
	this.active&&this.hide(this.active);
	var b=$(a.target),c=$(a.toggle);
...
e.hide=function(a){
	...
	$(".has-child a").on("click",function(a){
		...
		var b=$(a.target);
		(isMobile()&&b.is(".has-child > a")||b.is(".responsive-menu > .has-child > a"))&&a.preventDefault()});""==$(".category-description").text()&&$(".category-description").remove();
	b.prototype.showSlides=function(a){
		...
		$(".home-slider").each(function(){new b($(this))});
...
var k=localStorage.lastVisited,n=[];e=parseInt(e);e=isNaN(e)?null:e;k=parseInt(k);k=isNaN(k)?null:k;
var r=app.popupDuration?app.popupDuration:12;app.enablePopup&&k&&36E4>c-k&&(!e||c-e>36E5*r)&&n.push(1);
$(".ads").each(function(){
	n.push($(this).attr("data-position"))});
if(n.length){var q="device_type="+(isMobile()?1:3);
n.forEach(
	function(a){
		q+="&ads_position[]="+a
	});
...
/*$(".cmpr-product").autocomplete({
	source:function(a,b){
		$.ajax({url:"common/search_suggestion/product?keyword="+encodeURIComponent(a),
			dataType:"json",
			success:b
		})
	},
...
$(".form-cmpr").on("submit",function(a){
	...
	c=[];b.find(".prod-id").each(function(){var b=$(this);
		b.val()?c.push(b.val()):(a.preventDefault(),
			b.siblings("input").addClass("error"))
	});
...
e.setGroupVal=function(a,b){
	...
	e.update=function(a){
		...
		$(".countdown").each(function(){new f($(this))
		})
	});
