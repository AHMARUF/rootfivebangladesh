@php 
	$tr=Session::get('tran_id');
	$orders = DB::table('orders')->where('transaction_id',$tr)->first();
	$user_id = $orders->user_id;
	$invoice_id = $orders->invoice_id;

	$shoping = App\Models\Shoping::where('user_id',$user_id)->where('invoice_id',$invoice_id)->get();
	 $subtotal = App\Models\Shoping::all()->where('user_id',$user_id)->where('invoice_id',$invoice_id)->sum(function($t){
            ...
            <section class="top-content bb d-flex justify-content-between">
                ...
                <div class="top-left">
                    <div class="position-relative">
                        <p>Invoice No. <span>{{ $orders->invoice_id }}</span></p>
                    </div>
                </div>
            </section>
            ...
            <section class="store-user mt-5">
                <div class="col-12">
                    <div class="row bb pb-3">
                        ...
                        <div class="col-5">
                            <h2>{{ $orders->fastname }} {{ $orders->lastname }}</h2>
                            <p class="address">{{ $orders->address }} <br> {{ $orders->city }} <br>{{ $orders->zone }}</p>
                            <div class="txn mt-2">Number: {{ $orders->phone }}</div>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="balance-info">
                <div class="row">
                    ...
                    <div class="col-4 text-center">
                        @php
                            $data = strtoupper($orders->uni);
                            $data1 =$orders->invoice_id;
                            $fanal = $data1 ." ". $data;
                        @endphp
                    ...
                    <div class="col-4">
                        <table class="table border-0 table-hover">
                            <tr>
                                <td>Sub Total:</td>
                                <td>{{ number_format((float)($subtotal), 2) }}TK</td>
                            </tr>
                           {{--  <tr>
                                <td>Tax:</td>
                                <td>15$</td>
                            ...
                            <tfoot>
                                <tr>
                                    <td>Total:</td>
                                    <td>{{ number_format((float)( $orders->amount), 2) }}TK</td>
                                </tr>
                            </tfoot>
                        </table>
